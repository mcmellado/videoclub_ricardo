...
<body>
    ...
    $pdo = conectar();
    $sent = $pdo->query("SELECT * FROM peliculas ORDER BY codigo");
    ?>
            ...
            <main class="flex-1 grid grid-cols-3 gap-4 justify-center justify-items-center">
                <?php foreach ($sent as $fila) : ?>
                    <div class="p-6 max-w-xs min-w-full bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400"><?= hh($fila['genero']) ?></p>
            ...
            <?php if (!$carrito->vacio()) : ?>
                <aside class="flex flex-col items-center w-1/4" aria-label="Sidebar">
                    ...
                    <div>
                        ...
                        <a href="/alquilar.php" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-900">Alquilar</a>
                    </div>
                </aside>
            <?php endif ?>
        </div>
